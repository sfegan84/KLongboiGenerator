  ...
  gROOT->ProcessLine(Form(".L %s/Loader.C+", gSystem->Getenv("KLGEN")));
	...
	case sol3:
	  cout<<"Generating sol3"<<endl;
          usesol=1;
          in1.open(Form("%s/KLptoKpXi0_diffX_pol.dat",gSystem->Getenv("KLGEN"))); //note this solution is not binned in W, uses hyperon cm momentum instead
          SolXS = new TGraph2D();
          SolPY = new TGraph2D();
          SolXS->SetName("Sol2_XS");
          SolPY->SetName("Sol2_Py");
	  ...
	  case kl16:
        ...
        case kln3:
	      ...
	      if(Cocktail==false){
                ...
                charge.push_back(-1);
                charge.push_back(0);
                pdg_ID.push_back(PDGID_pizero);//this should be piminus
                pdg_ID.push_back(PDGID_lambd);
                mytree->Branch("cs",&cs);
                mytree->Branch("poly",&poly);
                mytree->Branch("costhetaK",&costhetaK);
    ...
    if (cascade0gen && usesol==1){//may need different parameterisation of Cascade decay, for now reuse charged mode
        lifetimef1=new TF1("lifetimef1","TMath::Exp(-x/([0]*[1]))",0, 5); //in ns tau= 2.9E-10
        lifetimef1->SetParameter(0, 0.1639);
    }
    double runtemp=0;
